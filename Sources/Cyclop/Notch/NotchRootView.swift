...
final class NotchRootView: NSView {
    ...
    override func hitTest(_ point: NSPoint) -> NSView? {
        ...
        // `contains` treats maxY as exclusive, and the pointer thrown at the
        // top of the display sits on exactly that edge. activeRect is grown
        // past it so the first check passes; AppKit's own hit test still
        // rejects the view. One point in, the content at the top is found.
        guard point.y >= bounds.maxY - 0.5 else { return nil }
        let probe = NSPoint(x: point.x, y: max(bounds.maxY - 1, bounds.minY))
        guard isReceivingDrag || activeRect.contains(probe) else { return nil }
        return super.hitTest(probe)
    }
