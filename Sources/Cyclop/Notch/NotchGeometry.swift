...
struct NotchGeometry {
    ...
    var railIconHeight: CGFloat {
        let icons = CGFloat(TabLayout.leftCapacity)
    ...
    /// How far the click target may reach past the housing, sideways only.
    /// Downward it stops at the menu bar — below that is the desktop.
    static let collapsedClickSidePad: CGFloat = 32

    /// What the menu bar actually occupies on this display, from the top edge
    /// down. The click target for a folded notch is this tall and no taller.
    var menuBarHeight: CGFloat {
        let measured = screen.frame.maxY - screen.visibleFrame.maxY
        return max(measured, notchSize.height, NSStatusBar.system.thickness)
    }

    /// Folded click target: as wide as the housing plus a miss-margin, as
    /// tall as the menu bar, flush with the top of the screen.
    var collapsedClickSize: CGSize {
        CGSize(width: notchSize.width + Self.collapsedClickSidePad * 2, height: menuBarHeight)
    }

    /// Concave shoulders of the folded silhouette.
    ///
    /// On a real cutout they live *inside* the measured width — the auxiliary
    /// gap already includes them. 6 pt was a drawn notch's decoration and
    /// sat outside the housing, a sliver of extra black on each side of the
    /// camera. A quarter of the height follows the 14/16" housing (8 pt on
    /// the 32 pt notch of a 2023 16" M2 Pro).
    var collapsedShoulderRadius: CGFloat {
        guard isPhysical else { return Theme.collapsedTopRadius }
        return min(10, (notchSize.height * 0.25).rounded())
    }

    /// Bottom corners of the folded silhouette. The real housing is a fuller
    /// round than the 9 pt drawn for a fake notch — about a third of the
    /// height, 11 pt on that same 32 pt cutout.
    var collapsedCornerRadius: CGFloat {
        guard isPhysical else { return Theme.collapsedBottomRadius }
        return min(12, (notchSize.height * 0.34).rounded())
    }
