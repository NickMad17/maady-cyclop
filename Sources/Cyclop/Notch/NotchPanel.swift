...
final class NotchPanel: NSPanel {
    ...
    /// The only frame this window may occupy. Window managers such as
    /// Rectangle talk through Accessibility and set `AXPosition` the way a
    /// drag would; `isMovable` does not stop that. A frame that is not this
    /// one is a foreign move and is refused.
    private(set) var pinnedFrame: CGRect = .zero

    func pin(to frame: CGRect) {
        pinnedFrame = frame
        super.setFrame(frame, display: false)
    }

    override func setFrame(_ frameRect: NSRect, display flag: Bool) {
        super.setFrame(pinnedOr(frameRect), display: flag)
    }

    override func setFrame(_ frameRect: NSRect, display displayFlag: Bool, animate animateFlag: Bool) {
        super.setFrame(pinnedOr(frameRect), display: displayFlag, animate: false)
    }

    override func setFrameOrigin(_ point: NSPoint) {
        super.setFrameOrigin(pinnedFrame.isEmpty ? point : pinnedFrame.origin)
    }
    ...
    private func pinnedOr(_ frame: CGRect) -> CGRect {
        pinnedFrame.isEmpty ? frame : pinnedFrame
    }

    @objc private func snapBackIfMoved(_ notification: Notification) {
        guard !pinnedFrame.isEmpty, self.frame != pinnedFrame else { return }
        super.setFrame(pinnedFrame, display: true)
    }
    ...
    init(contentRect: NSRect) {
        ...
        NotificationCenter.default.addObserver(
            self,
            selector: #selector(snapBackIfMoved),
