...
final class NotchScreenPanel {
    ...
    /// After a timer start or a colour pick the panel folds while the pointer
    /// is still on it. Hover would open it again on the next tick; this holds
    /// the fold until the pointer actually leaves.
    ...
    /// Folds now, even though the pointer is still on the panel. Used by the
    /// timer (the notch is the countdown) and the picker (the loupe cannot
    /// sample a panel that is covering the pixel). Hover stays deaf until
    /// the pointer leaves, so the fold is not undone on the next sample.
    func foldUntilPointerLeaves() {
        suppressOpen = true
        setOpen(false)
    }
    ...
    private func build() {
        ...
        state.onNotchClick = { [weak self] in self?.notchClicked() }

        panel.contentView = root
        panel.ignoresMouseEvents = true
        panel.pin(to: geometry.windowFrame)
        ...
        pointer.onChange = { [weak self] inside in
            ...
            if inside {
                guard vm.behavior.opensOnHover else { return }
                setOpen(true)
            } else {
                guard vm.behavior.closesOnLeave else { return }
                setOpen(false)
            }
        }
        ...
        if vm.behavior.opensOnHover,
           geometry.hoverRect(for: state.openBodySize).contains(NSEvent.mouseLocation) {
            pointer.setInside(true)
            setOpen(true)
        }
    }
    ...
    private func scheduleCollapseIfPointerAway() {
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.6) { [weak self] in
            ...
            if away, !holdsOpen, vm.behavior.closesOnLeave { setOpen(false) }
        }
    }
    ...
    private func applyActiveRect(open: Bool) {
        ...
        let size: CGSize
        if open {
            size = state.openBodySize
        } else if vm.behavior.opensOnClick {
            size = geometry.collapsedClickSize
        } else {
            size = geometry.collapsedSize
        }
        var rect = geometry.contentRect(for: size)
        var screen = geometry.contentScreenRect(for: size)
        if open {
            ...
            screen = screen.insetBy(dx: -Theme.openTopRadius, dy: 0)
        }
        // `contains` treats maxY as exclusive. The pointer thrown at the top
        // of the display sits on exactly that edge — the row above the drawn
        // housing — and would miss a rect that only covers the notch.
        if rect.maxY >= geometry.windowSize.height {
            rect.size.height += 2
        }
        rootView.activeRect = rect
        pointer.interactiveRect = screen
    }
}
