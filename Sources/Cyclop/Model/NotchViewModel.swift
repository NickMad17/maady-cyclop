...
final class NotchViewModel: ObservableObject {
    enum Tab: String, CaseIterable, Identifiable {
        case media, shelf, clipboard, snippets, calendar, timer, translate, notes, favorites, picker, teleprompter, settings
        ...
        var title: String {
            ...
            case .picker: return localized("Picker")
        ...
        /// Default rails, used only as the order a fresh install starts from.
        /// What is actually drawn comes from `TabLayout`: the first six that
        /// are on sit on the left, the rest on the right, Settings last.
        static let leftRail: [Tab] = [.media, .shelf, .clipboard, .snippets, .calendar, .timer]
        static let rightRail: [Tab] = [.picker, .translate, .notes, .favorites, .teleprompter, .settings]
    }
    ...
    @Published var tab: Tab = .media {
        didSet {
            ...
            // Same disk bargain as the shelf: a favorite from Desktop is a
            // TCC prompt waiting to happen, so the file is re-read here, with
            // the tab on screen, rather than at launch with nothing to explain it.
            if tab == .favorites {
                favorites.reload()
                favorites.refreshFromDisk()
            }
    ...
    /// Background work that is running right now. Start and stop are not
    /// cheap to call twice — the media helper would launch a second perl —
    /// so visibility flips go through here instead of calling them raw.
    private var running: Set<Tab> = []
    ...
    init() {
        ...
        self.picker = ColorPickerStore()
        self.layout = TabLayout()
        self.behavior = PanelBehavior()
        ...
        //
        // The timer is the exception that paints the folded notch, and it is
        // observed there directly, not through this list: twenty ticks a
        // second must not rebuild every pane.
        //
    ...
    func start() {
        ...
        snippets.reload()
        favorites.reload()
        ScreenshotVault.adoptSystemScreenshots()
        screenshotWatcher.onNewFile = { [weak self] url in
    ...
    func stop() {
        ...
        setRunning(.media, false, start: {}, stop: media.stop)
        setRunning(.clipboard, false, start: {}, stop: clipboard.stop)
        setRunning(.calendar, false, start: {}, stop: calendar.stop)
        // Whatever was typed makes it to disk even when quitting mid-thought.
        notes.flush()
    }

    /// Starts or stops the stores whose work is only worth doing while
    /// someone can open the tab that shows it. If the current tab was just
    /// hidden, lands on the first one still on the rail — Settings, if that
    /// list is empty.
    ...
    private func setRunning(_ tab: Tab, _ want: Bool, start: () -> Void, stop: () -> Void) {
        let isOn = running.contains(tab)
        if want, !isOn {
            start()
            running.insert(tab)
        } else if !want, isOn {
    ...
    func receivedScreenshot(at url: URL) {
        shelf.add([url])
        guard !isTyping, layout.isVisible(.shelf) else { return }
        tab = .shelf
    }
    ...
    func accept(urls: [URL]) -> Bool {
        ...
        // Desktop: switching to the shelf would steal the gesture that just
        // filled the empty pane. Files are ignored — this list is folders.
        if tab == .favorites {
            favorites.add(urls)
            return true
        }
